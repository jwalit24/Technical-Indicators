...
# Define function to calculate returns based on strategy
 def calc_strategy_profit(df):
    ...
    df['bullish_harami'] = ((df['Close'] > df['Open']) & (df['Close'].shift(1) < df['Open'].shift(1)) & (df['Open'] > df['Close'].shift(1)) & (df['Close'].shift(2) > df['Close'].shift(1)))
    # Calculate bearish harami pattern
    df['bearish_harami'] = ((df['Close'] < df['Open']) & (df['Close'].shift(1) > df['Open'].shift(1)) & (df['Open'] < df['Close'].shift(1)) & (df['Close'].shift(2) < df['Close'].shift(1)))
    # Calculate returns
    df['profit'] = df['Close']-df['Close'].shift(1)
    # Calculate strategy profit
    df['strategy_profit'] = np.where(df['bullish_harami'].shift(1) , df['profit'], np.where(df['bearish_harami'].shift(1), -df['profit'], 0))
