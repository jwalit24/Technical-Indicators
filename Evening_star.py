...
# Define function to calculate returns based on strategy
 def calc_strategy_profit(df):
    # Calculate evening pattern
    # df['evening_star'] = (((df['Close'] > df['Open']) & df['Close']>df['Open'].shift(2)) & ( (abs(df['Close'].shift(1)-df['Open'].shift(1))/df['Close'].shift(1))<0.0001) & (df['Open'].shift(2) > df['Close'].shift(2)))
    df['evening_star'] = np.where((df['Close'].shift(2) > df['Open'].shift(2)) & (df['Open'].shift(1) > df['Close'].shift(2)) & (df['Close'].shift(1) > df['Close'].shift(2)) & (df['Close'].shift(1) < df['Open'].shift(1)) & (df['Open'] < df['Close'].shift(1)) & (df['Close'] < df['Open'].shift(2)), 1, 0)
    # Calculate returns
    df['profit'] = df['Close']-df['Close'].shift(1)
    # Calculate strategy profit
    df['strategy_profit'] = np.where(df['evening_star'].shift(1) , df['profit'], 0)
